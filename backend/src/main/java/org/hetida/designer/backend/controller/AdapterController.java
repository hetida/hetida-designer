...
import org.hetida.designer.backend.exception.AdapterNotFoundException;
import org.hetida.designer.backend.exception.InvalidAdapterResponse;
...
public class AdapterController {
     ...
     * Returns the list of  adapters
     *
     * @return AdapterDTO List
     */
    @ApiOperation(
            value = "Returns metadata for the adapter with the given id",
            produces = MediaType.APPLICATION_JSON_VALUE,
            response = MetaDataDTO.class
    )
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "Successfully got metadata of adapter for given id"),
    ...
    public ResponseEntity<MetaDataDTO> getMetadata(@PathVariable("id") Integer id) {
        log.info("modify component {}", id);
        Adapter adapter = Adapter.getByID(id);
        if (adapter == null)
            throw new AdapterNotFoundException();
        MetaDataDTO metaData = adapterService.getMetaData(adapter);
        if (metaData == null){
            throw new InvalidAdapterResponse();
        }
