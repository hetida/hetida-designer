...
let
  ...
  init-components = writeShellScriptBin "init-hd-components" ''
      ...
      HETIDA_DESIGNER_BACKEND_API_URL=http://localhost:8080/api/ ${venvDirRuntime}/bin/python -c "from hetdesrun.exportimport.importing import import_transformations; import_transformations('./transformations', update_component_code=False);"
