...
def extract_sub_dict(data: dict[str, Any], json_path: list[str]) -> dict[str, Any] | None:
    """Extract a subdict of a nested dict structure

    Returns either the found dict, or None if it could not be found.
    """
...
def extract_sub_mapping(
    dict_of_metadata: dict[str | int, dict[str, Any]], key: str
) -> dict[str | int, Any]:
    return {main_key: sub_dict.get(key) for main_key, sub_dict in dict_of_metadata.items()}


def add_columns_from_metric_metadata(
    multitsframe: pd.DataFrame,
    keys: list[str] | None = None,
    replace_metric_col_with_key: str | None = None,
    ...
    metrics_metadata = extract_sub_dict(multitsframe.attrs, ["by_metric"])

    if metrics_metadata is None:
        metrics_metadata = extract_sub_dict(multitsframe.attrs, ["metric_metadata"])
    ...
    if replace_metric_col_with_key is not None:
        multitsframe["metric"] = multitsframe["metric"].replace(
            extract_sub_mapping(metrics_metadata, replace_metric_col_with_key)  # type: ignore
        )
    if keys is not None:
        for key in keys:
            mapping_dict = extract_sub_mapping(metrics_metadata, key)  # type: ignore

            mapping_dict = defaultdict(lambda: default_value, mapping_dict)

            multitsframe[key] = multitsframe["metric"].map(mapping_dict)
