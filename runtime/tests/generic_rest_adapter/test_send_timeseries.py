        ...
        with mock.patch(
            ...
            with pytest.raises(
                ...
                await send_data(
                    {
                        "outp_5": FilteredSink(
                            ref_id="sink_id_5", type="timeseries(float)"
                        )
                    },
                    {"outp_5": ts_5},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            with pytest.raises(
                AdapterOutputDataError, match="does not have UTC timezone"
            ):
                await send_data(
                    {
                        "outp_6": FilteredSink(
                            ref_id="sink_id_6", type="timeseries(float)"
                        )
                    },
                    {"outp_6": ts_6},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            ts_7 = pd.Series(
                ["not a float"], index=[pd.Timestamp("2019-08-01T15:45:36Z")]
            )
            with pytest.raises(AdapterOutputDataError, match="Expected float"):
                await send_data(
                    {
                        "outp_7": FilteredSink(
                            ref_id="sink_id_7", type="timeseries(float)"
                        )
                    },
                    {"outp_7": ts_7},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            ts_8 = pd.Series(
                ["not an int"], index=[pd.Timestamp("2019-08-01T15:45:36Z")]
            )
            with pytest.raises(AdapterOutputDataError, match="Expected int"):
                await send_data(
                    {
                        "outp_8": FilteredSink(
                            ref_id="sink_id_8", type="timeseries(int)"
                        )
                    },
                    {"outp_8": ts_8},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            ts_9 = pd.Series(
                ["not a bool"], index=[pd.Timestamp("2019-08-01T15:45:36Z")]
            )
            with pytest.raises(AdapterOutputDataError, match="Expected bool"):
                await send_data(
                    {
                        "outp_9": FilteredSink(
                            ref_id="sink_id_9", type="timeseries(bool)"
                        )
                    },
                    {"outp_9": ts_9},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            with pytest.raises(AdapterOutputDataError, match="Expected string"):
                await send_data(
                    {
                        "outp_10": FilteredSink(
                            ref_id="sink_id_10", type="timeseries(string)"
                        )
                    },
                    {"outp_10": ts_10},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
            ...
            with pytest.raises(AdapterOutputDataError, match="Did not receive Pandas Series"):
                await send_data(
                    {
                        "outp_11": FilteredSink(
                            ref_id="sink_id_11", type="timeseries(float)"
                        )
                    },
                    {"outp_11": no_ts},
                    adapter_key="test_end_to_end_send_only_timeseries_data_adapter_key",
