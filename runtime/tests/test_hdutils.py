...
def test_modify_timezone_good_dataframe(dataframe):
    local_summertime = modify_timezone(dataframe, to_timezone="Europe/Berlin", column_name="timestamp")

    #German summer time starts in last Sunday in March at 2 am. --> UTC 1am
    timestamp_id = local_summertime.columns.get_loc("timestamp")
    assert local_summertime.iloc[1, timestamp_id].utcoffset() == datetime.timedelta(seconds=3600)
...
def test_modify_timezone_good_series(series_summer, series_winter):
    ...
    np.testing.assert_array_equal(
        pd.to_timedelta(local_summertime.index[1:]-local_summertime.index[:-1]).total_seconds().values,
        [3600., 3600., 3600.])
...
def test_modify_timezone_no_tz_known(series_summer):
    ...
    with pytest.raises(TypeError):
        _ = modify_timezone(
            series_summer.index.tz_localize(None),
...
def test_modify_timezone_no_tz_in_index(series_summer):
    ...
    with pytest.raises(TypeError):
        _ = modify_timezone(
            series_summer.reset_index(inplace=True),
            to_timezone="Europe/Berlin")
