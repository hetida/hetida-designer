...
def walk_thing_nodes(
    ...
    for snk in response_obj["sinks"]:
        metadata = client.get(f'/adapters/localfile/sinks/{snk["id"]}/metadata/').json()
        for metadatum in metadata:
            snk_attached_metadata_dict[(snk["id"], metadatum["key"])] = metadatum

    metadata_tn = client.get(
...
async def test_resources_offered_from_structure_hierarchy():
    ...
    response_obj = client.get("/adapters/localfile/structure").json()
    ...
    for src in all_srcs:
        response_obj = client.get(f'/adapters/localfile/sources/{src["id"]}').json()
        for key in src.keys():
            assert response_obj[key] == src[key]

    for snk in all_snks:
        response_obj = client.get(f'/adapters/localfile/sinks/{snk["id"]}').json()
        for key in snk.keys():
            print(response_obj)
            assert response_obj[key] == snk[key]

    for tn in all_tns:
        response_obj = client.get(f'/adapters/localfile/thingNodes/{tn["id"]}').json()
        for key in tn.keys():
            print(response_obj)
    ...
    for ((src_id, key), md) in src_attached_metadata_dict.items():
        response_obj = client.get(
            f"/adapters/localfile/sources/{src_id}/metadata/{key}"
        ).json()
    ...
    for ((snk_id, key), md) in snk_attached_metadata_dict.items():
        response_obj = client.get(
            f"/adapters/localfile/sinks/{snk_id}/metadata/{key}"
        ).json()
        print(response_obj, "versus", md)
        assert response_obj["key"] == key
        assert response_obj["value"] == md["value"]
