...
def test_import_with_name_and_tag_filter(clean_test_db_engine):
    patched_session = sessionmaker(clean_test_db_engine)
    with mock.patch(
        "hetdesrun.persistence.dbservice.nesting.Session",
        patched_session,
        ...
        with mock.patch(
            ...
            with mock.patch("hetdesrun.utils.requests.put", return_value=response_mock):
                ...
                persisted_transformations = select_multiple_transformation_revisions()

                assert len(persisted_transformations) == 2

                assert (
                    persisted_transformations[0].name,
                    persisted_transformations[0].version_tag,
                ) in names_and_tags
                assert (
                    persisted_transformations[1].name,
                    persisted_transformations[1].version_tag,
                ) in names_and_tags
