...
async def test_resources_offered_from_structure_hierarchy(async_test_client):
    ...
    async with async_test_client as client:
        ...
        for ((src_id, key), md) in src_attached_metadata_dict.items():
            response_obj = (
                await client.get(f"/adapters/blob/sources/{src_id}/metadata/{key}")
            ).json()
            ...
            if md.get("isSink", False):
                ...
                resp = await client.post(
                    f"/adapters/blob/sources/{src_id}/metadata/{key}", json=md
                )
                assert resp.status_code == 200

        for ((snk_id, key), md) in snk_attached_metadata_dict.items():
            response_obj = (
                await client.get(f"/adapters/blob/sinks/{snk_id}/metadata/{key}")
            ).json()
            print(response_obj, "versus", md)
            assert response_obj["key"] == key
            assert response_obj["value"] == md["value"]
            ...
            if md.get("isSink", False):
                assert response_obj["isSink"]
                resp = await client.post(
                    f"/adapters/blob/sinks/{snk_id}/metadata/{key}", json=md
                )
                assert resp.status_code == 200
        ...
        for ((tn_id, key), md) in tn_attached_metadata_dict.items():
            ...
            if md.get("isSink", False):
                assert response_obj["isSink"]
                resp = await client.post(
                    f"/adapters/blob/thingNodes/{snk_id}/metadata/{key}", json=md
                )
                assert resp.status_code == 200
        ...
        for src in all_srcs:
            if src["type"].startswith("metadata"):
                response_obj = (
                    await client.get(
                        f'/adapters/blob/thingNodes/{src["thingNodeId"]}/metadata/{src["metadataKey"]}'
                    )
                ).json()
        ...
        for snk in all_snks:
            if snk["type"].startswith("metadata"):
                response_obj = (
                    await client.get(
                        f'/adapters/blob/thingNodes/{snk["thingNodeId"]}/metadata/{snk["metadataKey"]}'
                    )
                ).json()
