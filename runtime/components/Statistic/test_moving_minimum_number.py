...
import numpy as np
import pytest
from .moving_minimum_number import main
...
def test_numeric_index():
    assert main(data=pd.Series({0: 1.0, 1: 4.0, 3: 4.0, 4: 7.0}), n=3)["movmin"].equals(
        pd.Series({0: np.nan, 1: np.nan, 3: 1.0, 4: 4.0})
    )


def test_numeric_unsorted():
    assert main(data=pd.Series({3: 4.0, 0: 1.0, 1: 4.0, 4: 7.0}), n=3)["movmin"].equals(
        pd.Series({0: np.nan, 1: np.nan, 3: 1.0, 4: 4.0})
    )


def test_series_empty():
    assert main(data=pd.Series(dtype=float), n=4)["movmin"].empty
