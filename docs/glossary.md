...
* **Adapter**: A small piece of software that provides access to data sources or data sinks in order to make them available for execution of workflow revisions. Typically, [adapters](./adapter_system/intro.md) connect to databases (SQL, NoSQL (e.g. timeseries databases)), blob storage, files, external APIs and more. The base installation comes with several [builtin adapters](./adapter_system/builtin_adapters.md). You can of course write your own adapter implementations.
