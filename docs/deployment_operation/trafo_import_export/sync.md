...
The **hdctl** Bash command line tool provides a useful `sync` feature that allows to sync fine-granular selections from a hetida designer instance to a local directory and vice-versa.
...
## Installation
For syncing besides Bash, GNU core utils and curl, [jq](https://jqlang.github.io/jq/) is required. If these preconditions are fulfilled you can just copy the [hdctl script file](https://github.com/hetida/hetida-designer/blob/release/hdctl) to your working environment.

See the [hdctl script](https://github.com/hetida/hetida-designer/blob/release/hdctl) for more detailed installation instructions.
...
!!! note
    A full description of available parameters / settings can be viewed in [openapi.json](https://github.com/hetida/hetida-designer/blob/release/runtime/openapi.json) (use an API viewer!). See [API](../../integration_guide/api.md).
...
Since these options actually edit the component code during export you may be affected by [pitfalls concerning reproducibility and deserializability](../../user_guide/repr_pitfalls.md) when pushing them back or to another instance.
...
!!! warning
    Overwriting released components is dangerous: For example it may destroy workflows depending on them! In particular if inputs or ouputs are added/removed. It circumvents reproducibility.
...
See [](../../user_guide/component_writing/unit_testing.md).
