...
Components and workflows can be exported and imported flexibly and with a fine granularity via the backend API `/api/transformations` GET and PUT endpoints. To make that easier, scripts are available that can be run from any Bash environment or from the runtime Docker image.
...
The hetida designer backend [API](../../integration_guide/api.md) exposes a GET and a PUT endpoint for transformations. This endpoints provide fine-granular filter functionality and hence can be used for exporting / importing.
...
For example, the shell command to export the components that return the mathematical constants e and π is
...
If you (re-)start the backend container, you can use the [base trafo autodeployment feature](./base_trafo_deployment.md).
