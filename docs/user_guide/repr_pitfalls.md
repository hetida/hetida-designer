...
* Sending components to a hetida designer instance using an action that overwrites released transformation revisions. For example by activating the `allow_overwrite_released` flags during [import](./trafo_import_export/import_export.md), [autoimport](./trafo_import_export/autoimport.md) or via some of the [maintenance](../deployment_operation/maintenance/) actions, e.g. from using [hdctl](../deployment_operation/trafo_import_export/sync.md).
