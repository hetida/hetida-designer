...
You can of course establish the *analytical* steps of model management like for example automatic performance measuring / evaluation as part of hetida designer workflows.

But the *management* part, i.e. selecting the current/prod model based on evaluation results, fallback procedures on declining performance and so on has to be implemented around the systems which [invoke](../integration_guide/trafo_exec_guide/trafo_execution_overview.md) hetida designer from the outside.
