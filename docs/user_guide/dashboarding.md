...
- The dashboard renders:
  ...
  - Dataframe / multitsframe / singletsframe outputs as a sortable and filterable table.
...
- You can override all time ranges of the wiring in use with a single absolute or relative time range selected in the dashboard.
  ...
  - Note that time ranges only affect those inputs — typically of type SERIES, MULTITSFRAME or SINGLETSFRAME — that are wired to sources from adapters which actually provide a time range filter. In particular, manual input is not affected.
