...
- In practice you would set/configure the wiring directly in your web application's front- or backend and call the execution endpoint.
- See [hetida platform](https://hetida.io/en/) for an example of data platform integrating hetida designer in such a way, that plots generated by hetida designer can be shown on customizable dashboards.
- The dashboarding here is provided as pure html with inline styles and pure javascript while loading additional js libraries and CSS from external CDNs. Typically one would prefer to use a modern frontend framework with plotly wrapper components and accompanying build system and so on. On the other side the approach here makes the resulting dashboard pages self-contained / downloadable as single html pages.
...
Dashboards require login according to the Backend auth settings (see [auth docs](../deployment_operation/authentication.md) for details). See the section about configuring [auth for dashboarding](../deployment_operation/authentication.md#dashboarding) for the required additional configurations.
