...
!!! info "Components importing other components"
    ...
    Note that components can in fact import other components, see [component tips](../component_writing/index.md). So the bottom component layer can have some depth in itself.
...
Note that then the hdctl command line tool can be used to [sync](../../deployment_operation/trafo_import_export/sync.md) components (and workflows) between your local files and a hetida designer instance. There also is an import button on the bottom left corner of the "home" tab where a component can be imported by simply pasting its source code.
...
```
A wiring does not have to wire all outputs of a transformation revision: Unwired outputs are automatically wired to the [direct_provisioning](../../integration_guide/adapter_system/builtin_adapters/direct_provisioning.md) adapter which means the results are simply returned as part of the response to the execution request. There are also "shorter" [uri wirings](../../integration_guide/trafo_exec_guide/uri_wirings.md).

### Adapter
[Adapters](../../integration_guide/adapter_system/adapter_system_introduction.md) are pieces of software that provide access to data sources or data sinks in order to make them available for execution of transformation revisions.
...
For the later an adapter has to provide an [API](../../integration_guide/adapter_system/adapter_rest_api_interface.md).

The base installation comes with several [built-in adapters](../../integration_guide/adapter_system/builtin_adapters/index.md) and a demo adapter that demonstrates how to [develop your own adapters from scratch](../../integration_guide/adapter_system/custom_adapters/).
...
In the hd user interface execution can be triggered via the test execution button. This opens a dialog where you can specify a wiring, including directly providing values via [manual input](../../integration_guide/adapter_system/builtin_adapters/direct_provisioning.md).

<figure markdown="span">
![demo adapter wiring in exec dialog](../../assets/wiring_from_demo_adapter_exec_dialog.png){ width="700" }
</figure>

In production, execution is often triggered regularly (scheduled) or event-based by other systems / services. Typically this is done by sending http requests to one of the corresponding backend [execution API endpoints](../../integration_guide/trafo_exec_guide/execution_via_api.md) like `/api/transformations/execute`.
...
Another option for event-based execution is [Kafka execution](../../integration_guide/trafo_exec_guide/execution_via_kafka.md) or, with a focus on streaming use cases, the [Kafka consumption mode](../../integration_guide/trafo_exec_guide/kafka_consumption_mode.md).

See [Running in production tutorial](../../integration_guide/trafo_exec_guide/trafo_execution_overview.md) for more details.
