...
To process wirings with virtual structure adapter sources and sinks, an additional step in the execution pipeline of the hetida designer was introduced. Before the execution input object (WorkflowExecutionInput) is prepared, all virtual structure adapter related wiring information is resolved to the referenced source or sink.
