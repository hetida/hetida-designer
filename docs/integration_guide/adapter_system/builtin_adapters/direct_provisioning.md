...
For simple types (like STRING, FLOAT, INT) you can enter data directly in the input field. For SERIES, DATAFRAME, MULTITSFRAME, SINGLETSFRAME or ANY inputs clicking on the input field opens an editor dialog where json-data can be entered (a template is provided in the input text field).

Note that for SERIES, DATAFRAME, MULTITSFRAME and SINGLETSFRAME you can add metadata (which is attached to pandas `.attrs` attribute) by using the wrapped json format described [here](../../../user_guide/attached_metadata.md). This also allows to specifiy some json parsing options for these types and may be required, e.g. when you want to enter/send data with duplicate index entries.
...
A [SINGLETSFRAME](../../../user_guide/data_types/singletsframes.md) uses the same format as a MULTITSFRAME, just without the `metric` column, since it holds exactly one metric. Every column besides `timestamp` is a value dimension:
...
Analogously to MULTITSFRAME you can obtain this format from a Pandas DataFrame variable `df` via

```python
print(df.to_json(orient="columns", date_format="iso", indent=2))
```

There is a wrapped format as well — note that it uses `"DATAFRAME"` as `__hd_wrapped_data_object__`, just like MULTITSFRAME. hetida designer always outputs the wrapped format for SINGLETSFRAME objects with default parsing options.
