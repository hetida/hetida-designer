...
![](../../../assets/pegelonline-component.png){width=300}
</figure>
...
- have inputs `timestampTo` and `timestampFrom` if its type is of one of the types requiring these filters (e.g. MULTITSFRAME, SINGLETSFRAME, SERIES)
...
Especially relevant for component adapter source components for MULTITSFRAME, SINGLETSFRAME or SERIES:

For the `timestampFrom` and `timestampTo` required inputs of type STRING: As shown above, with the following code they can be provided as absolute timestamps as well as [dtexp](https://github.com/stewit/dtexp) relative expressions and will be resolved with the execution timestamp as "now":
