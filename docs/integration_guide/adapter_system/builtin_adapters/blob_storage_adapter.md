...
This [file](https://github.com/hetida/hetida-designer/blob/release/runtime/demodata/blob_storage_adapter_hierarchy.json) contains an example of such a json starting with:
...
The environment variable `BLOB_STORAGE_REGION_NAME` should be set to the region name matching your blob storage setup. The default value is "eu-central-1".
...
!!! note
    The python module tensorflow (or keras) is not included in the standard hetida designer dependencies. To use tensorflow in the hetida designer components or workflows, you need to [add this dependency](../../../deployment_operation/managing_python_dependencies.md). The blob storage adapter is compatible with tensorflow version 2.12.0 or later.
...
![blob storage source](../../../assets/blob_storage_adapter_assign_source.png){height="630" width=530}
</figure>
...
<figure markdown="span">
![blob storage sink](../../../assets/blob_storage_adapter_assign_sink.png){height="630" width=530}
</figure>

### Self-defined classes

When pickling objects in Python, at import time classes and references must be importable using the exact same module path.

Naively, this can be achieved by having components that output classes as objects via an ANY output that then must be used as operator both in the workflow that stores the object as well as in the workflow that loads the stored object. For more than one class, a dictionary with one entry per class can be used.

However: A better and preferred approach is to import relevant classes from one component in another component.
...
MyClass = my_other_component.MyClass
...
For example, an inference / prediction component can import the model class from a training component. Even if it does not actually initialize the object, but gets it loaded from the blob storage adapter, this ensures that the class is available at the expected import module path!

### Blob Storage Adapter wirings
