# Adapter API Interface Specification
...
You may also have a look at an example OpenAPI description by running the [docker-compose setup](../../get_started.md) and then access the Python Demo Adapter OpenAPI docs at: http://localhost:8092/docs
...
These endpoints enable user interfaces (like the hetida designer test execution dialog) to construct wirings. They provide information on which data sources and sinks are available. All adapters should implement these endpoints if wirings for them are meant to be configured in user interfaces.
...
So the generic rest adapter implementation must then provide/accept data at that endpoint accordingly.

In general it is of course necessary that a generic rest adapter implementation actually provides/accepts data at the appropriate endpoints if it offers sources/sinks for them.
...
A [SINGLETSFRAME](../../user_guide/data_types/singletsframes.md) is a single, possibly multi-dimensional timeseries. The endpoint works exactly like `/multitsframe`, except that the records have **no** `metric` field, since all records belong to one and the same metric.

Query parameters:

- `id`: required exactly once: This is a source id of a singletsframe source occurring in the structure endpoint
- `from`: The timestamp from which on datapoints of the source are requested. Isoformat, up to nanosecond precision. E.g. 2020-03-11T13:45:18.194000000Z" or "2025-11-11T02:03:15+00:00".
- `to`: Analogous to the `from` query parameter, the timestamp until which datapoints of the source are requested.
- `job_id`: Runtime may send a job id (UUID) which the adapter implementation can e.g. use for logging to identify log messages belonging to the same execution across services.
...
```json
{"timestamp": "2020-03-11T13:45:18.194000000Z", "value": 42.3, "state": "ok"}
{"timestamp": "2020-03-11T14:45:18.237000000Z", "value": 41.7, "state": "ok"}
{"timestamp": "2020-03-11T15:45:18.081000000Z", "value": 18.4, "state": "suspicious"}
```

This response will always have a `timestamp` entry plus at least one further entry. Each entry besides `timestamp` is a value dimension of the timeseries — in the example above `value` and `state`. `timestamp` may not be `null`, must be an ISO-8601 timestamp and should always have UTC timezone.

!!! info "no metric field"
An entry named `metric` is not forbidden, but it is _not_ interpreted as a metric — it would simply be another value dimension. If your data has several metrics, offer it as a `multitsframe` source instead.

##### Attaching metadata to the singletsframe

Analogous to `/multitsframe`, the response is allowed to send a header `Data-Attributes` containing a base64 encoded UTF8-encoded JSON String representing the metadata that is attached to the Pandas DataFrame objects' `attrs` attribute.
...
- `id`: required exactly once: This is a sink id of a singletsframe sink occurring in the structure endpoint
- `job_id`: Runtime may send a job id (UUID) which the adapter implementation can e.g. use for logging to identify log messages belonging to the same execution across services.
...
```json
[
  {"timestamp": "2020-03-11T13:45:18.194000000Z", "value": 42.3, "score": 0.7}
  {"timestamp": "2020-03-11T14:45:18.237000000Z", "value": 41.7, "score": 0.2}
]
```

Same rules as in the corresponding GET endpoint apply here, only timestamp handling is different.

##### Retrieving attached singletsframe metadata
