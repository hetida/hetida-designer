# Executing Transformation via Backend API
...
Additionally, the latest (by release timestamp!) released revision of a revision group can be executed through the POST web service endpoint
...
For a full description we refer to the [openapi.json](https://github.com/hetida/hetida-designer/blob/release/runtime/openapi.json).
...
<figure markdown="span">
![](../../assets/workflow_revision_id.png){ width="400" }
</figure>

#### Input and output wirings

See the section 'Enumeration "type"' in the [specification of webservice endpoints of adapters](../adapter_system/adapter_rest_api_interface.md) for the possible values of "type".
...
For both web service endpoints, a successful response contains the result values for those workflow outputs without wiring, since this implies that they are wired with the [direct provisioning adapter](../adapter_system/builtin_adapters/direct_provisioning.md). Also hetida designer internal types for each output are provided. Outputs wired via adapters do not occur in the response.
...
Furthermore the response can contain detailed error information including tracebacks in the case of exceptions as well as some metrics on execution processing time, loading/sending data times and loaded / dent data amounts. For a full description we refer to the [API](../api.md).
